...
static void write_tagged_page(uint32_t index)
{
   ...
   nandlog_store_record(STORAGE_TYPE_IMU, 500 * index, page_buffer, TEST_RECORD_DATA_BYTES);
   nandlog_flush(true);
}
...
static bool verify_tagged_page(uint32_t expected_index, uint32_t length)
{
   ...
   if (length != nandlog_data_bytes_per_page())
   {
...
// nandlog_disable() based on RTC validity, and the RTC is typically unset in a bare test, so writes must
...
static void test_forced_block_crossings(void)
{
   ...
   print("Writing %u pages of %u bytes (%u pages/block, %u boundaries crossed)\n",
         (uint32_t)BLOCK_CROSSING_NUM_PAGES, (uint32_t)nandlog_data_bytes_per_page(),
...
//
// Covers the Phase 2 change to nandlog_flush(true). Before Phase 2 a partial page was written WITHOUT
...
static void test_partial_page_flush(void)
{
   ...
   nandlog_store_record(STORAGE_TYPE_MOTION, 12345, page_buffer, PARTIAL_TEST_PARTIAL_BYTES);
   if (!nandlog_has_buffered_data())
   {
...
//
// Every other test calls nandlog_begin_reading(0, 0), which skips the seek entirely, so without this the
...
static void test_time_range_seek(void)
{
   ...
   // nandlog_begin_reading() takes the same experiment-relative milliseconds the page headers carry; the
...
//
// Covers nandlog_retrieve_page_by_seq(), the primitive the host will use to ask for pages that arrived
...
static void test_timestamp_jump(void)
{
   ...
   nandlog_store_record(STORAGE_TYPE_IMU, 10000, record, sizeof(record));
   nandlog_store_record(STORAGE_TYPE_IMU, 11000, record, sizeof(record));
   nandlog_store_record(STORAGE_TYPE_IMU,  3000, record, sizeof(record));   // <-- the jump
   nandlog_store_record(STORAGE_TYPE_IMU,  4000, record, sizeof(record));
   nandlog_flush(true);
   ...
   for (uint32_t i = 0; i < num_chunks; ++i)
   {
      ...
      const uint32_t length = nandlog_retrieve_next_page(verify_buffer, &header);
...
static void store_range_record(uint32_t timestamp, uint8_t range_mm_div)
{
   ...
   nandlog_store_record(STORAGE_TYPE_RANGES, timestamp, data, sizeof(data));
}
...
static void store_time_anchor(uint32_t network_ms, uint32_t local_ms)
{
   ...
   nandlog_store_record(STORAGE_TYPE_TIME_ANCHOR, network_ms, &local_ms, sizeof(local_ms));
}
...
static void test_time_anchor_recovery(void)
{
   ...
   nandlog_store_record(STORAGE_TYPE_VOLTAGE, 1000, voltage, sizeof(voltage));
   ...
   nandlog_store_record(STORAGE_TYPE_VOLTAGE, expected_ms + 5000, voltage, sizeof(voltage));
...
static void test_page_retransmission(void)
{
   ...
   {
      nandlog_page_header_t header;
      const uint32_t length = nandlog_retrieve_page_by_seq(wanted[i], verify_buffer, &header);
      if (!length)
      {
   ...
   nandlog_page_header_t header;
   if (nandlog_retrieve_page_by_seq(RETRANSMIT_TEST_PAGES + 500, verify_buffer, &header))
   {
...
//
// Probes the one Phase 0 path that can destroy data: nandlog_init() recovers the write head heuristically,
