...
bool storage_init(void)
{
   ...
   return chip_present;
#else
   // Startup continues even when the identity never matched, as it always has
   (void)chip_present;
#endif

   // Configure the chip and load, or on a first boot build, its bad-block table
   nandlog_chip_init();
...
bool storage_store_metadata(const void *blob, uint16_t length)
{
   ...
   for (uint32_t attempt = 0; !success && (attempt < metadata_ring_pages); ++attempt)
   {
      ...
      success = nandlog_chip_write_page(transfer_buffer, slot) && nandlog_chip_read_page(transfer_buffer, slot) &&
                meta_header_valid((const storage_meta_header_t*)transfer_buffer);
      // Deliberately do NOT retire the block here
   }
   if (!success)
   {
...
void storage_store_record(uint8_t record_type, uint32_t timestamp, const void *data, uint32_t data_length)
{
   ...
   {
      if ((page_last_timestamp - timestamp) <= STORAGE_TIMESTAMP_TOLERANCE_MS)
