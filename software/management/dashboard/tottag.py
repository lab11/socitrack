...
def unpack_datetime(time_zone, start_timestamp, timestamp):
   ...
   if start_timestamp is None:
      local_datetime = pytz.utc.localize(datetime.datetime.fromtimestamp(timestamp, datetime.timezone.utc)).astimezone(pytz.timezone(time_zone))
      date_string = local_datetime.strftime('%m/%d/%Y')
      time_string = local_datetime.strftime('%H:%M')
      seconds_string = local_datetime.strftime('%S')
   else:
      offset = pytz.utc.localize(datetime.datetime.fromtimestamp(start_timestamp, datetime.timezone.utc)).astimezone(pytz.timezone(time_zone)).utcoffset().total_seconds()
