...
def get_off_and_on_charger_times(data, label, peak_width=50, window_size=10, visualize=True):
    ...
    for key in combined:
        local_time = pytz.utc.localize(datetime.fromtimestamp(key.timestamp(), timezone.utc)).astimezone(pytz.timezone(tzlocal.get_localzone_name())).strftime('%Y-%m-%d %H:%M:%S %Z')
